...
<?php
    ...
    if (isset($_POST["knop"])){
        ...
        if ($artikelaantal > 0){
            $sql = '
                UPDATE tblbestellingen 
                SET artikelaantal = '.$artikelaantal.' 
                WHERE bestelnummer = '.$bestelnummer.'
            ';
        } else {
            $sql = '
                DELETE FROM tblbestellingen
                WHERE bestelnummer = '.$bestelnummer.'
            ';
        }
            ...
            echo '
                <div class="w3-content" style="max-width:1100px">
                    <div class="w3-container w3-padding-64" id="contact">
                        <h1>Succes</h1><br>
                        <p>Bestelling succesvol wijzigd, klik <a href="winkelkar_kijken.php">hier</a> om terug te gaan.</p>
                    </div> 
                </div>
            ';
        } else {
        ...
        $sql = 'SELECT * FROM tblbestellingen INNER JOIN tblartikels ON (tblartikels.artikelnummer = tblbestellingen.artikelnummer) WHERE bestelnummer = '.$_GET["teverwijderen"].'';
        $resultaat = $mysqli->query($sql);
        $row = $resultaat->fetch_assoc();
        ...
        echo '
            <div class="w3-content" style="max-width:1100px">
                <div class="w3-container w3-padding-64" id="contact">
                <h1>Wijzig bestelling '.$row["bestelnummer"].'</h1><br>
                <form method="post" action="winkelkar_verwijderen.php" enctype="multipart/form-data">
                    <input type="hidden" name="bestelnummer" value="'.$row["bestelnummer"].'">
                    <p>Naam: '.$row["artikelnaam"].'</p>
                    <p>Prijs: '.$row["artikelprijs"].'</p>
                    <p><input class="w3-input w3-padding-16" type="number" placeholder="Aantal" required name="artikelaantal" min="0" value="'.$row["artikelaantal"].'"></p>
                    <p><button class="w3-button w3-light-grey w3-section" type="submit" name="knop">Wijzig aantal</button></p>
                </form>
                </div>
            </div>
        ';
